...
function ambition_widgets_init()
{
	...
	register_sidebar( array(
		'name' 				=> __( 'Testimonial Page Section', 'ambition' ),
		'id' 					=> 'ambition_testimonial_page_our_client_sidebar',
		'description'   	=> __( 'Shows widgets on Testimonial Page Template. Suitable widget: TH: Testimonial', 'ambition' ),
	...
	register_sidebar( array(
		'name' 				=> __( 'Services Page Section', 'ambition' ),
		'id' 					=> 'ambition_services_page_section',
		'description'   	=> __( 'Shows widgets on Services Page Template. Suitable widget: TH: Services', 'ambition' ),
		'before_widget' 	=> '<div id="%1$s" class="widget %2$s">',
		'after_widget'  	=> '</div>',
		'before_title'  	=> '<h2 class="widget-title">',
		'after_title'   	=> '</h2>',
	...
	register_sidebar( array(
		'name' 				=> __( 'Our Team Page Section', 'ambition' ),
		'id' 					=> 'ambition_ourteam_page_section',
		'description'   	=> __( 'Shows widgets on Our Team Page Template. Suitable widget: TH: Our Team Recommended image size ( 300 * 300 )px.' , 'ambition' ),
		'before_widget' 	=> '<section id="%1$s" class="widget %2$s">',
		'after_widget'  	=> '</section>',
		'before_title'  	=> '<h2 class="widget-title">',
		'after_title'   	=> '</h2>',
	...
	register_widget("ambition_ourteam_widget");
}
/****************************************************************************************/
/**
		...
		while ($get_featured_pages->have_posts()):
			...
			<div class="container clearfix<?php if($featured_display == 2){ echo ' opp'; }elseif($featured_display == 3){ echo ' midd'; } else{ echo ''; } ?>">
				<div class="featured-page-content">
					...
					<?php } ?>
					<?php the_excerpt(); ?>
					<a title="<?php the_title_attribute(); ?>" href="<?php the_permalink(); ?>" class="call-to-action"><?php echo esc_attr($excerpt_text); ?></a>
				</div>
...

{
	...
	function form($instance)
	{
		$instance           = wp_parse_args((array) $instance, array('checkbox' => '','checkbox_largeview' => '','number' => '6','page_id0'=>'','page_id1'=>'','page_id2'=>'','page_id3'=>'','page_id4'=>'','page_id5'=>'','page_id6'=>'','page_id7'=>''));
		...
		<p>
			...
			<?php _e( 'Number of Services:', 'ambition' ); ?>
			</label>
		...
		$checkbox_largeview = esc_attr($instance['checkbox_largeview']);?>
		<p>
			...
			<label for="<?php echo $this->get_field_id('checkbox'); ?>"><?php _e('Check to display content as left','ambition'); ?></label>
		</p>
		<p>
			<input id="<?php echo $this->get_field_id('checkbox_largeview'); ?>" name="<?php echo $this->get_field_name('checkbox_largeview'); ?>" type="checkbox" value="1" <?php checked( '1', $checkbox_largeview ); ?>/>
			<label for="<?php echo $this->get_field_id('checkbox_largeview'); ?>"><?php _e('Check to display large image','ambition'); ?></label>
		</p>
	...
	function update($new_instance, $old_instance)
	{
		...
		$instance['checkbox_largeview'] = strip_tags($new_instance['checkbox_largeview']);
	...
	function widget($args, $instance)
	{
		...
		$checkbox_largeview    = esc_attr('checkbox_largeview', empty($instance['checkbox_largeview'])?'':$instance['checkbox_largeview'], $instance);
		extract($args);
		extract($instance);
		...
		echo $before_widget; ?>
			<div class="container clearfix">
				<div class="column <?php if ( $instance['checkbox'] == true ) { echo "display-left"; } ?> <?php if ( $instance['checkbox_largeview'] == true ) { echo "large-view"; } ?> clearfix">
				<?php
					$j = 1;
				...
				<?php
					if (has_post_thumbnail()) {
						if ( $checkbox_largeview == true ) { ?>
						<div class="service-img"><a title="<?php the_title(); ?>" href="<?php the_permalink(); ?>"> <?php echo get_the_post_thumbnail( $post->ID, 'ambition-recent-work'); ?> </a>
						</div>
						<?php	}else{ ?>
					...
					the_permalink(); ?>" class="more-link"><?php echo esc_attr($excerpt_text); ?></a>
					<?php }else{
						echo $excerpt; ?>
					</article>
					<?php	}
...

{
	...
	function form($instance)
	{
		...
		<p>
			<label for="<?php echo $this->get_field_id('number'); ?>">
			<?php _e( 'Number of Recent Work:', 'ambition' ); ?>
			</label>
			<input id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo $number; ?>" size="3" />
		</p>
		<p>
...
?>
		<p>
			<label for="<?php echo $this->get_field_id('number'); ?>">
				<?php _e( 'Number of Testimonial:', 'ambition' ); ?>
			</label>
			<input id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo $number; ?>" size="3" />
			<input type="submit" name="savewidget" id="widget-ambition_testimonial-3-savewidget" class="button button-primary widget-control-save right" value="Save">
		...
		<p>	
			<?php
			_e('Testimonial Description '.$i, 'ambition'); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id('company_name'); ?>">
				<?php _e('Company Name '.$i, 'ambition'); ?>
			</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('company_link'); ?>">
				<?php _e('Company Link '.$i, 'ambition'); ?>
			</label>
...
* Our Team widget
*/
class ambition_ourteam_widget extends WP_Widget {

	function ambition_ourteam_widget() {
		$widget_ops = array( 'classname' => 'widget_our_team', 'description' => __( 'Display Our Team ( Our Team Layout ) recommendation size ( 300 * 300 )px', 'ambition' ) );
		$control_ops = array( 'width' => 200, 'height' =>250 ); 
		parent::__construct( false, $name = __( 'TH: Our Team', 'ambition' ), $widget_ops, $control_ops);
	}
	function form( $instance ) {
		$instance = wp_parse_args( (array) $instance, array( 'title' => '', 'number' => '4', 'image1' => '', 'text1' => '', 'name1' =>'', 'designation1'=>'','image2'=>'', 'text2'=>'','name2'=>'','designation2'=>'','image3' => '', 'text3' => '', 'name3' =>'', 'designation3'=>'','image4' => '', 'text4' => '', 'name4' =>'', 'designation4'=>'' ) );
		$title = strip_tags($instance['title']);
		$number = absint( $instance[ 'number' ] );
		for ( $i=1; $i<=$number; $i++ ) {
			$image = 'image'.$i;	
		...
		<p>
			<label for="<?php echo $this->get_field_id('number'); ?>">
				<?php _e( 'Number of OurTeam:', 'ambition' ); ?>
			</label>
			<input id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo $number; ?>" size="3" /> 
			<input type="submit" name="savewidget" id="widget-ambition_ourteam_widget-2-savewidget" class="button button-primary widget-control-save right" value="Save">			<span class="spinner" style="display: none;"></span>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>">
				<?php _e( 'Title:', 'ambition' ); ?>
		...
		<?php for ( $i=1; $i<=$number; $i++ ) {
			...
			<?php _e( 'OurTeam Description '.$i,'ambition'); ?>
	...
	function widget( $args, $instance ) {
		extract($args);
		global $ambition_theme_default;
		$options = $ambition_theme_default; 
		$title = empty( $instance['title'] ) ? '' : $instance['title'];
...
if( !empty( $image )  || !empty( $name ) || !empty( $designation ) || !empty( $text ))  {
		...
		<?php 
		echo '</div><!-- .container -->';
		echo $after_widget .'<!-- .widget_our_team -->';
	}
} ?>