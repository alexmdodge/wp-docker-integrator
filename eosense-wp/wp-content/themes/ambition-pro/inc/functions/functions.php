<?php
/**
 * Ambition Pro functions and definitions
 *
...
function ambition_scripts_styles_method() {
	...
	if( $responsive_layout == 'on' ) {
		...
		wp_enqueue_style('ambition-responsive', get_template_directory_uri().'/css/responsive.css');
	}
	/**
	...
	wp_enqueue_script('jquery_cycle', get_template_directory_uri().'/js/jquery.cycle.all.min.js', array('jquery'), '2.9999.5', true);
		$ambition_google_font_loads = array();
		...
		if( $ambition_fontfamily_content != 'Lato' ) :
		array_push( $ambition_google_font_loads,$ambition_settings['ambition_fontfamily_content'] );
		endif;

		if( $ambition_fontfamily_navigation != 'Lato') :
		array_push( $ambition_google_font_loads,$ambition_settings['ambition_fontfamily_navigation'] );
		endif;
		if( $ambition_fontfamily_titles_heading != 'Lato') :
		array_push( $ambition_google_font_loads,$ambition_settings['ambition_fontfamily_titles_heading'] );
		endif;

		if( 'Lato' == $ambition_fontfamily_content || 'Lato' == $ambition_fontfamily_navigation ):
		array_push( $ambition_google_font_loads,'Lato');
		endif;

  	$ambition_google_font_load = implode("|", $ambition_google_font_loads);
	...
	if ((is_home() || is_front_page() || $ambition_slider_status =='allpage') && empty($disable_slider)) {
...
function ambition_excerpt_length($length) {
	...
	global $excerpt_text;
	$excerpt_length = $ambition_settings['ambition_excerpt_length'];
	$excerpt_text = $ambition_settings['ambition_excerpt_text'];
	return $excerpt_length;// this will return 50 words in the excerpt
}
...
function ambition_alter_home($query) {
	...
	$featured_post_page_slider = array();
	...
	for( $i = 1; $i <= $ambition_settings['ambition_slide_no']; $i++ ){
		if( isset ( $ambition_settings['featured_post_page_slider_' . $i] ) && $ambition_settings['featured_post_page_slider_' . $i] > 0 ){
					...
					$featured_post_page_slider	=	array_merge( $featured_post_page_slider, array( $ambition_settings['featured_post_page_slider_' . $i] ) );
		}

	}
		if($ambition_settings['exclude_slider_post'] == 1 &&  $disable_setting == 0){
			if ($query->is_main_query() && $query->is_home()) {
				$query->query_vars['post__not_in'] = $featured_post_page_slider;
				$query->query_vars['category__in'] = $ambition_settings['ambition_categories'];
			}
		}
}
